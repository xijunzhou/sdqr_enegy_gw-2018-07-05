	...
	if(type == DLT645)
		{
			...
			res = dlt645_decode(&buff[head],len,&Elect);
			if(res == -1)
				return res;
			else if(res == -2)
				return res;
...
#if 0
			else if(-1 != dlt645_decode1(&buff[head],len,&Elect3))
				{
...
int protocol_meter_ctl_decode(unsigned char *buff,int size)
{
	int i,type,res = -1;
	int len = size;
	int head = 0;
	type = decode(buff,size,&head,&len);
	if(type == DLT645)
		return dlt645_ctl_decode(&buff[head],len,&Elect);
	return -1;	
}
