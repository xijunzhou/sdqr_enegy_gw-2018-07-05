...
#define COLLECT_ENERGY_FORWARDA_ONLY 1

#define APP_NAME "meter"
#define APP_VERSION "V1.0"

#define ARCHIVE_CONFIG_FILE "/var/meter/meter_config"
#define METER_DB "/var/meter/meter.db"
...
static pthread_mutex_t cloud_mutex = PTHREAD_MUTEX_INITIALIZER;//初始化上行云端锁

static pthread_mutex_t device_mutex = PTHREAD_MUTEX_INITIALIZER;//初始化下行设备锁
...
char FeeControl[50] = {'\0'};
char FeeControlResult[50] = {'\0'};
...
void meter_work(void);
void meter_control_task(void);
...
#define ARCHIVE_COL 3
char *archive_column[ARCHIVE_COL];
/*
 *读取value
 */
int load_conf(void)
{
  ...
  printf("\nGet configure file from %s!",ARCHIVE_CONFIG_FILE);
  fp = fopen(ARCHIVE_CONFIG_FILE,"r");
  if(fp == NULL)
  	{
  		printf("\nCan't open configure file: %s!",ARCHIVE_CONFIG_FILE);
  	return -1;//文件打开错误
  }
  	
  rc = sqlite3_open(METER_DB,&db);   //打开数据库  
  if(rc != SQLITE_OK)  
  {  
        printf("zErrMsg = %s\n",zErrMsg);  
...
***************************************************************/
  memset(sql,'\0',sizeof(sql));  
  sprintf(sql,"delete from archive;");//删除所有记录
	ret = sqlite3_exec(db,sql,NULL,NULL,&zErrMsg);   //执行sqlite命令语句  
	if(ret != SQLITE_OK)
		{
			printf("zErrMsg = %s\n",zErrMsg);
...
***************************************************************/	
    ...
    {
    	if(get_item_from_line(line,archive_column))
    		{
    			/**************************************************************
					插入新的档案
					***************************************************************/
					  memset(sql,'\0',sizeof(sql));  
						sprintf(sql,"insert into archive values(%d,'%s','%s');",
						                                    atoi(archive_column[0]),
						                                    archive_column[1],
						                                    archive_column[2]);//插入数据  
						 printf("\n%s",sql);                                 
						 ret = sqlite3_exec(db,sql,NULL,NULL,&zErrMsg);   //执行sqlite命令语句  
						 if (ret != SQLITE_OK)
						  {
...
int gateway_init(void)
{
  ...
  ret = db_init();
 	...
 	ret = load_conf();          //从/var/meter/meter_config文件中获取表档案,并存入 数据库表archive
 	if(ret < 0)
 		return ret;
 		
	ret = loader_archive();        //装载数据库表 archive
	if( ret < 0)
 		return ret;	
  ...
  sprintf(FeeControl,"FeeControl");
  sprintf(FeeControlResult,"FeeControlResult");
...
/*-----------------------------------------------------------------------start
20180508 为演示系统定制数据帧，只采集正向有功总电能、开合闸状态 两个值*/
void update_elect_meter_data_state(void)
{
	int len,data = 0;
	char *pstr = NULL;
	time_t now;
...
//Trip state
  len = sprintf(value,"%d",Elect.trip);
  value[len] = '\0';
  json_john1(elect_meter_buff,"Trip",value);
  strcat(elect_meter_buff,"}");
...
int publish_topic(const char* topic,const char* value)
{
   ...
   printf("\nPublish %s:%s Successfull!",topic,value);
   return 0;  	
}

int publish_data(int type)
{
	if(type == 0x10)
		return publish_topic("water_meter",water_cold_meter_buff);
	
	else if(type == 0x30)
		return publish_topic("gas_meter",gas_meter_buff);
	
	else if(type >= 0x20 && type <= 0x21)
		return publish_topic("heat_meter",heat_meter_buff);
	
	else if(type == 0x40)
		return publish_topic("elect_meter",elect_meter_buff);
	
	else if(type == 0x41)
		return publish_topic("elect3_meter",elect3_meter_buff);
}
...
int call_device(device_t *dev)
{
	...
	switch(type)
	{
		...
		case 0x21:	
		case 0x30:		
			len = cj188_encode(&buff[0],dev->id,type);   //CJ188 编码
			if(len == -1)
			{
				printf("\n cj188 encode fail!");
				return -1;
			}		
	    size = write(fd_meter, &buff[0],len);       //编码 发送指令
	    if(size != len)
	    {
	    	printf("\r\nwrite deivce erro!");
	    	return -1;
      ...
      len = dlt645_encode(&buff[0],dev->id,elect_data_type);
			if(len == -1)
			{
				printf("\n DLT645 encode fail!");
				return -1;
...
20180508 为演示系统定制数据帧，只采集正向有功总电能、开合闸状态 两个值*/		
		  ...
		  ret = meter_read();	
			...
			elect_data_type = READ_TRIP;    /*读取开合闸状态*/	
...
device_t *select_device_from_rule(int now)
{
  ...
  if(now >= gateway.dev_num)
  	{
  		printf("\n Waring: Cann't get device :%d/%d",now,gateway.dev_num);
  		gateway.now = 0;
		  gateway.next = 0;
			return NULL;
		}
...
功能：电表控制命令
cmd:
  0 跳闸、1 合闸允许、2 合闸、3 报警、4 报警接触、5 保电、6 保电解除
  
  主机跳闸命令：
   mosquitto_pub -h 118.24.14.213 -t FeeControl -m "{\"id\":\"428386000003\",\"trip\":\"1\"}"

*/
int elect_meter_ctl(device_t *dev,int cmd)
{
	int size,i;
  int type;
  int ret,len = 0;
	...
	len = dlt645_ctl_encode(&buff[0],dev->id,cmd);
 ...
 Meter_ReadBuff[size] = '\0'; 
 printf("\r\nelect_meter_ctl   Recived %d bytes from Meter:",size);	
 for(i=0;i<size;i++)
		 {
		 	  //Meter_ReadBuff[i] = (Meter_ReadBuff[i] & 0xff);
		 		printf("0x%02x ",Meter_ReadBuff[i]);
		 	}
 ret = protocol_meter_ctl_decode(Meter_ReadBuff,size);                                //解码
 if(ret == 0)
 	{
 		//成功
...
成功：meter.trip = 0 合闸
      meter.trip = 1 开闸
*/
int get_elect_meter_trip(device_t *dev)
{
	int ret,len,size = 0;
	char buff[METER_BUFFER_SIZE] = {'\0'};
	len = dlt645_encode(&buff[0],dev->id,READ_TRIP);
  if(len == -1)
			{
				printf("\n DLT645 encode fail!");
				return -1;
...
void handle_device(void)
{
  ...
  pthread_mutex_lock(&device_mutex);//获得rs485锁
  tcflush(fd_meter,TCIOFLUSH);  //清空串口输入输出缓存 
  type = call_device(device);
  pthread_mutex_unlock(&device_mutex);//释放rs485锁
...
int update_rule(void)
{
  ...
  printf("\n\n-----------------------------------------update rule start,now cycle = %d",rule_update_cycle);
  for(i=0;i<gateway.dev_num;i++)
  {
  	 memset(sql, '\0', sizeof(char) * 2000);
  ...
  printf("\n-----------------------------------------update rule end,now cycle = %d",rule_update_cycle);
  free(sql); 
  sql = NULL;
...
/****
  * Function: void meter_control_task(void)
  *@Describe: 费控进程，负责接收云平台费控命令，下发命令控制表进行开合闸后，返回表开合闸状态！
  *@Param   : 无
  *@Return  : 空
  */
...
int main(void)
{
	...
	pthread_t tid_meter_ctl;   //创建一个费控线程，负责接收云平台费控命令，下发命令控制表进行开合闸后，返回表开合闸状态！
  
  pthread_t tid_meter_work;  
  
  sleep(60);
  
  if(io_init()) 	return -1;
  if(gateway_init()) 	return -1;
  	
 	
  /*主线程 */
  if(pthread_create(&tid_meter_work,NULL,(void *)meter_work,NULL)!=0)
  {
         printf("创建主线程失败!\n");
         exit(1);
  }	
  ...
  /*采集线程 */
  if(pthread_create(&tid_meter_ctl,NULL,(void *)meter_control_task,NULL)!=0)
  {
         printf("创建费控线程失败!\n");
         exit(1);
  }
  
  pthread_join(tid_meter_ctl,NULL);
  pthread_join(tid_meter_work,NULL);
...
int put_system(const char* command)
{		
    FILE *fp;
    int ret = -1;
    fp = popen(command, "w");
...
int handle_system_reset(void)
{
	...
	time_t now = time(NULL);	
	p = localtime(&now);
	if((p->tm_hour == 23 && p->tm_min > 57)||(p->tm_hour == 6 && p->tm_min > 57)||(p->tm_hour == 16 && p->tm_min > 57))
			put_system("reboot");
  return 0;
}
...
void meter_control_task(void)
{
	...
	char trip[20] = {'\0'};	
	char buf[1024] = {'\0'};
	const char *topic = "FeeControl";
	while(1)
	{		
		
		memset(buf,'\0',sizeof(buf));
    sprintf((char *)buf,"mosquitto_sub -h %s -p %s -t \"%s\"",MQTT_HOST,MQTT_PORT,topic);
    printf("\n%s\n",buf);
    ...
    memset(buf,'\0',sizeof(buf));
    while(fgets(buf,1024-1,fpRead)!=NULL) //{"id":"428386000003","trip":"1"} or {"id":"428386000003","trip":"0"}
    {
    		printf("\n====================Clould Command:\n %s",buf);		
				...
				//获取id值
			  json = cJSON_GetObjectItem(sub, "id");
			  if(json == NULL)
			  	break;
			 
			  strcpy(id,json->valuestring);
			  
			  printf("\nid = %s",id);	
			  //获取trip值
			  json = cJSON_GetObjectItem(sub, "trip");
			  if(json == NULL)
			  	break;
			  
			  strcpy(trip,json->valuestring);
			  printf("\ntrip = %s",trip);
        
        now = time(NULL);
				for(i=0;i<gateway.dev_num;i++)
				{
				//	printf("\nid=%s,gateway.dev[i].id=%s",id,gateway.dev[i].id);
					if(strcmp(id,gateway.dev[i].id) == 0)
						{
							
							gateway.next = i;           //将查询位置指向当期电表
							gateway.dev[i].collect_time = now+10; //设置时间，让主线程在下一秒采集该电表状态并推送给云平台
							dev = &gateway.dev[i];
							break;
						}
				}
				if(i >= gateway.dev_num)    //无此电表，返回，继续接收订阅的费控命令
					{
						continue;
					}
        ...
        tcflush(fd_meter,TCIOFLUSH);  //清空串口输入输出缓存 
				if(strcmp(trip,"1") == 0)	
					ret = elect_meter_ctl(dev,TRIP_OPEN);
				else if(strcmp(trip,"0") == 0)	
					ret = elect_meter_ctl(dev,TRIP_CLOSE);
				else 
					ret = -1;				
				
				if(ret == 0)                      //电表执行命令成功，返回 0
					{
						sprintf((char *)buf,"{\"id\":\"%s\",\"Res\":\"0\"}",id);
            publish_topic("FeeControlRes",buf);
         //   get_elect_meter_trip(dev);      //执行查询开合闸状态命令，改命令需延时查询，否则只能查到“继电器命令状态(0通，1断)”，“继电器状态”并未更新。当期设计：将该电表采集时间值零，在主线程立即采集。
					}
				else if(ret == 1)                //电表执行命令不成功，返回 1
					{
						sprintf((char *)buf,"{\"id\":\"%s\",\"Res\":\"1\"}",id);
            publish_topic("FeeControlRes",buf);
					}
				else                            //电表执行命令异常返回或超时，返回 2
					{
						sprintf((char *)buf,"{\"id\":\"%s\",\"Res\":\"2\"}",id);
            publish_topic("FeeControlRes",buf);
					}
				pthread_mutex_unlock(&device_mutex);//释放rs485锁			
    ...
    pclose(fpRead);
    sleep(60);   //出现网络连接异常，mosquitto_sub断开连接后，1分钟重连接！
	}
	pthread_exit(NULL);
	meter_hw_close();
}